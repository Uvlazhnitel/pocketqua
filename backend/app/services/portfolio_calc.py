...
def build_portfolio_snapshot(db: Session) -> dict:
    ...
    for symbol, row in by_symbol.items():
        ...
        if not price:
            warnings.append(f"Missing EUR price for {symbol}; excluded from valuation")
            continue
        row["value_eur"] = row["amount"] * price.price_eur

    total = sum(row["value_eur"] for row in by_symbol.values())

    assets_out: list[dict] = []
    for row in sorted(by_symbol.values(), key=lambda x: x["symbol"]):
        value = row["value_eur"]
        weight = (value / total) if total > 0 else 0.0
        assets_out.append({"symbol": row["symbol"], "value_eur": value, "weight": weight})

    return {"total_value_eur": total, "assets": assets_out, "warnings": warnings}
